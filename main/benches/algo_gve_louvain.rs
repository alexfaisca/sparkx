...
use tool::{
    communities::gve_louvain::AlgoGVELouvain,
...
{
    // construct + compute
    let _algo = AlgoGVELouvain::<'a, EdgeType, Edge>::new(g).expect("algo should succeed");
}
...
{
    let mut group = c.benchmark_group("time_and_edges_throughput_gve_louvain");
    ...
    for (label, path) in datasets {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(*label), path, |b, _p| {
            ...
            b.iter_batched(
                || {
                    let algo = AlgoGVELouvain::<EdgeType, Edge>::new_no_compute(&graph)
...
fn bench_generic<EdgeType, Edge>(
    ...
    Edge: GenericEdge<EdgeType> + Send + Sync + 'static,
{
    let mut group = c.benchmark_group(group.to_string() + "_gve_louvain");
    group
        .sample_size(60)
        .confidence_level(0.99)
        .noise_threshold(0.01);
