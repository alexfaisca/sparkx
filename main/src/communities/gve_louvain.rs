    ...
    AlgoGVELouvain<'a, EdgeType, Edge>
{
    ...
    fn scan_communities(
        ...
        for e_idx in *index.get(2 * u)..*index.get(2 * u + 1) {
            let v = edges.get(e_idx).dest();
            if !self_allowed && v == u {
                continue;
            }
            // 𝐻𝑡[𝐶'[v]] ← 𝐻𝑡[𝐶'[v]] + w
            *map.entry(*communities.get(v)).or_insert(0) += 1;
        }
        Ok(())
    }
    ...
    fn best_community_linked_to_u(
        ...
        // Ku->d
        let k_u_d = map.get(&c_u).cloned().unwrap_or(0) as f64;
        // Σd
        let sig_d = sigma.get(c_u).load(Ordering::Relaxed) as f64;
        for (&c_v, &edge_weight) in map.iter() {
            // Ku->c
    ...
    fn louvain_move(
        l_pass: usize,
        prev_comm_count: usize,
        k: SharedSliceMut<AtomicUsize>,
        sigma: SharedSliceMut<AtomicUsize>,
        affected: SharedSliceMut<AtomicBool>,
        cur_index: SharedSliceMut<usize>,
        cur_edges: SharedSliceMut<(usize, usize)>,
        mut coms: SharedSliceMut<usize>,
        mut new_coms: SharedSliceMut<usize>,
        m: f64,
        tolerance: f64,
        threads: usize,
            ...
            thread::scope(|scope| -> Result<(), Box<dyn std::error::Error>> {
                ...
                for tid in 0..threads {
                    ...
                    thread_res.push(scope.spawn(move |_| {
                        ...
                        if l_pass != 0 && i == 0 {
                            for u in start..end {
                                let deg_u = *cur_index.get(u * 2) - *cur_index.get(u * 2 + 1);
                        ...
                        for u in start..end {
                            ...
                            for e_idx in idx_start..idx_end {
                                let v = cur_edges.get(e_idx);
                                if v.0 == u {
                                    continue; // ignore self-loop if any
                                }
                                let comm_v = *coms.get(v.0);
                                // accumulate weight to neighbor's community
                                *neighbor_map.entry(comm_v).or_insert(0) += v.1;
                            }
                            ...
                            if best_comm != c_u && best_gain > 1e-12 {
                                ...
                                *coms.get_mut(u) = best_comm;
                                ...
                                for e_idx in idx_start..idx_end {
                                    let v = cur_edges.get(e_idx);
                                    if v.0 != u {
                                        affected.get(v.0).store(false, Ordering::Relaxed);
                                    }
                                }
                            }
                        }
    ...
    fn compute(&mut self, mmap: u8) -> Result<(), Box<dyn std::error::Error>> {
                        ...
                        *gdi.get_mut(out_offset + 1) = u_index_end;
                        ...
                        let deg_u = match u_index_end.overflowing_sub(u_index_start) {
                            ...
                            (r, true) => {
                                return Err(
                                    format!(
                                        "overflow for {u} index ends at {u_index_end} but starts at {u_index_start} so sub is {r}"
                                        ).into()
                                    );
                            },
                        };
                        let edges = match graph_ptr.slice(u_index_start, u_index_end) {
                            Some(e) => e.iter().map(|e| (e.dest(), 1)).collect::<Vec<(usize, usize)>>(),
                            None => { return Err(format!("error getting edges for {u} in initialization").into()); }
                        };
                        gde.write_slice(
                            u_index_start,
                            edges.as_slice(),
        ...
        let affected = affected.shared_slice();
            ...
            // fill edges of new graph
            {
                ...
                for u in 0..prev_comm_count {
                    ...
                    for e_idx in u_start..u_end {
                        ...
                        let pos = *next_index.get(cu * 2) + *new_coms.get(cu);
                        *next_edges.get_mut(pos) = (cv, v.1);
                        *new_coms.get_mut(cu) += 1;
                    }
                }
            }
